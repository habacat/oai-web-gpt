...
export default function ChatList() {
    ...
    const {
        state: { selectedChat, user_uuid, userLoggedIn},
        dispatch
    } = useAppContext()
    ...
    return (
        <div className='flex-1 mb-[48px] mt-2 flex flex-col overflow-y-auto'>
            {groupList.map(([date, list]) => {
                return (
                    <div key={date}>
                        <div className='sticky top-0 z-10 p-1.5 text-xs font-bold rounded-md text-oaigray2 bg-oaiwhite dark:bg-oaiblack dark:text-oaigray'>
                            {date}
                        </div>
                        <ul>
                            {list.map((item) => {
